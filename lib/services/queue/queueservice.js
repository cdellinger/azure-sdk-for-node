...
QueueService.prototype.deleteMessage = function (queue, messageid, popreceipt, optionsOrCallback, callback) {
  ...
  var processResponseCallback = function (responseObject, next) {
    var finalCallback = function (returnObject) {
      console.log('DELETE MESSAGE RESPONSE VALUE: ' + returnObject.response === undefined ? false : returnObject.response.isSuccessful);
      if (returnObject.response == undefined){
        console.log(JSON.stringify(returnObject.error));
        console.log(JSON.stringify(returnObject.response));
      }
      //callback(returnObject.error, (returnObject.response === undefined ? false : returnObject.response.isSuccessful), returnObject.response);
      callback(returnObject.error, returnObject.response.isSuccessful, returnObject.response);
    };
